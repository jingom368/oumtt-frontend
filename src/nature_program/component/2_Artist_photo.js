...
function ArtistPhoto(props) {    
    ...
    artist_image.push(
        <picture key={artist_info.name}>
            ...
            <img className={artist_class + imagechange} alt="" src={require('../../assets/img/' + artist_group + '/' + artist_info.image + pngchange)} 
            onClick={() => SetActive()}/>
        </picture>
    );
    ...
    artist_text.push(<div key={artist_info.team}>{artist_info.team}</div>);
    artist_text.push(<div key={artist_info.name}>{artist_info.name}</div>);
    return (
